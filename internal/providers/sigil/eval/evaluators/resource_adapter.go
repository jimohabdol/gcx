...
// StaticDescriptor returns the resource descriptor for Sigil evaluators.
...
// NewTypedCRUD creates a TypedCRUD for Sigil evaluators.
func NewTypedCRUD(ctx context.Context) (*adapter.TypedCRUD[eval.EvaluatorDefinition], string, error) {
	...
	if err != nil {
		return nil, "", fmt.Errorf("failed to load REST config for Sigil evaluators: %w", err)
	}
...
// NewLazyFactory returns an adapter.Factory for Sigil evaluators.
