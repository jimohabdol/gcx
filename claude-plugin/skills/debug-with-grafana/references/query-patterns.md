...
gcx datasources prometheus query 'up'
...
gcx datasources prometheus query <uid> 'up'

# CPU usage by job
gcx datasources prometheus query <uid> 'avg by(job) (rate(cpu_usage_seconds[5m]))'

# Memory usage with threshold
gcx datasources prometheus query <uid> 'node_memory_MemAvailable_bytes < 1000000000'
```
...
gcx datasources prometheus query <uid> 'rate(http_requests_total[5m])' \
  --from now-1h --to now --step 1m

# CPU usage for specific time period
gcx datasources prometheus query <uid> 'avg(cpu_usage)' \
  --from 2026-03-01T00:00:00Z --to 2026-03-01T12:00:00Z --step 5m

# Disk usage over last 24 hours
gcx datasources prometheus query <uid> 'disk_used_percent' \
...
gcx datasources prometheus query <uid> 'rate(requests[1m])' \
  --from now-5m --to now --step 1s

# Medium ranges: 1-5 minute steps
gcx datasources prometheus query <uid> 'rate(requests[5m])' \
  --from now-6h --to now --step 1m

# Long ranges: 15-60 minute steps
gcx datasources prometheus query <uid> 'rate(requests[1h])' \
...
gcx datasources prometheus query <uid> 'sum(http_requests_total)'

# Average by label
gcx datasources prometheus query <uid> 'avg by(job) (cpu_usage)'

# Top 5 by value
gcx datasources prometheus query <uid> 'topk(5, http_requests_total)'

# 95th percentile
gcx datasources prometheus query <uid> 'histogram_quantile(0.95, rate(http_request_duration_seconds_bucket[5m]))'
```
...
gcx datasources prometheus query <uid> 'up' -o graph

# Range query as graph
gcx datasources prometheus query <uid> 'cpu_usage' --from now-6h --to now --step 5m -o graph
```
...
```bash
# All logs from a job
gcx datasources loki query <loki-uid> '{job="varlogs"}'

# Multiple labels (AND)
gcx datasources loki query <loki-uid> '{job="varlogs",level="error"}'

# Regex matching
gcx datasources loki query <loki-uid> '{job=~"mysql.*",level!="debug"}'

# Exclude specific values
gcx datasources loki query <loki-uid> '{namespace="production",pod!~"test.*"}'
```
...
```bash
# Contains text
gcx datasources loki query <loki-uid> '{job="varlogs"} |= "error"'

# Doesn't contain text
gcx datasources loki query <loki-uid> '{job="varlogs"} != "debug"'

# Regex match in log line
gcx datasources loki query <loki-uid> '{job="varlogs"} |~ "error|exception"'

# JSON parsing
gcx datasources loki query <loki-uid> '{job="varlogs"} | json | level="error"'
```
...
# Specific time range
gcx datasources loki query <loki-uid> '{namespace="prod"}' \
  --from 2026-03-01T00:00:00Z --to 2026-03-01T12:00:00Z
```
...
gcx datasources prometheus labels -d <uid> --label instance
...
# List scrape targets
gcx datasources prometheus targets -d <uid>
```
...
```bash
gcx datasources prometheus query <uid> 'up{job="prometheus"}'
```

4. Explore available metrics for that job:
```bash
gcx datasources prometheus metadata -d <uid> | grep -i <keyword>
```
...
```bash
# List all available labels
gcx datasources loki labels -d <loki-uid>

# Get values for specific label
gcx datasources loki labels -d <loki-uid> --label job
gcx datasources loki labels -d <loki-uid> --label namespace

# Find series matching selectors
gcx datasources loki series -d <loki-uid> -M '{job="varlogs"}'
gcx datasources loki series -d <loki-uid> -M '{namespace="production"}' -M '{level="error"}'
```
...
```bash
gcx datasources loki query <loki-uid> '{job="varlogs",namespace="prod"}'
```
...
```bash
gcx datasources loki query <loki-uid> '{job="varlogs"}' --from now-5m --to now
# Output:
...
```bash
gcx datasources loki query <loki-uid> '{job="varlogs"}' --from now-5m --to now -o wide
# Output:
...
```bash
gcx datasources prometheus query <uid> 'up' -o json
```
...
```bash
gcx datasources prometheus query <uid> 'up' -o yaml
```
...
gcx datasources prometheus query <uid> 'up' -o json | jq '.data.result[].metric.job'

# Filter results
gcx datasources prometheus query <uid> 'up' -o json | jq '.data.result[] | select(.value[1] == "1")'

# Count results
gcx datasources prometheus query <uid> 'up' -o json | jq '.data.result | length'
```
...
UP=$(gcx datasources prometheus query $DS_UID 'up{job="critical-service"}' -o json | \
...
for query in "${QUERIES[@]}"; do
  echo "Query: $query"
  gcx datasources prometheus query $DS_UID "$query" --from now-5m --to now -o graph
  echo "---"
done
```
...
gcx datasources prometheus query <uid> 'cpu_usage' --from now-24h --to now --step 1m -o json > cpu-data.json
...
```bash
# Slow
gcx datasources prometheus query <uid> 'http_requests_total'

# Fast
gcx datasources prometheus query <uid> 'http_requests_total{job="api",status="200"}'
```

2. **Choose appropriate range selectors**:
```bash
# For rate queries, match range to step
gcx datasources prometheus query <uid> 'rate(requests[5m])' --step 5m

# Don't use huge ranges for instant queries
gcx datasources prometheus query <uid> 'rate(requests[5m])'  # Good
gcx datasources prometheus query <uid> 'rate(requests[1h])'  # Usually unnecessary
```

3. **Limit time ranges**:
```bash
# Query only what you need
gcx datasources prometheus query <uid> 'up' --from now-1h --to now  # Good
gcx datasources prometheus query <uid> 'up' --from now-30d --to now  # Slow
```
...
gcx datasources prometheus query <uid> 'up{job="critical-service"}' | grep "1"
```
...
```bash
# HTTP error rate
gcx datasources prometheus query <uid> 'rate(http_requests_total{status=~"5.."}[5m])' \
  --from now-1h --to now --step 1m -o graph
```
...
```bash
# Memory usage by pod
gcx datasources prometheus query <uid> 'container_memory_usage_bytes{namespace="production"}' -o graph
```
...
gcx datasources prometheus query <uid> 'rate(requests[5m])' --from now-1h --to now -o json > now.json
gcx datasources prometheus query <uid> 'rate(requests[5m])' --from now-25h --to now-24h -o json > yesterday.json
```
