...
gcx datasources prometheus metadata -d <uid> -o json > metrics.json

# 3. Search for HTTP-related metrics
grep -i http metrics.json

# 4. Get metadata for specific HTTP metric
gcx datasources prometheus metadata -d <uid> --metric prometheus_http_requests_total

# 5. Test query
gcx datasources prometheus query <uid> 'rate(prometheus_http_requests_total[5m])'
```
...
gcx datasources prometheus query <uid> 'up{job="my-service"}'

# 4. List available status codes
gcx datasources prometheus labels -d <uid> --label code
```
...
gcx datasources prometheus labels -d <uid> --label app
```
...
gcx datasources loki series -d <loki-uid> -M '{namespace=~"kube.*"}'

# 6. Multiple label filters
gcx datasources loki series -d <loki-uid> -M '{job="varlogs", namespace="default"}'
```
...
gcx datasources prometheus labels -d $PROM_UID
gcx datasources loki labels -d $LOKI_UID
```
...
gcx datasources prometheus metadata -d <uid> -o json | jq '.data | to_entries[] | select(.value[0].type=="counter")'

# Search for metrics containing "http"
gcx datasources prometheus metadata -d <uid> -o json | jq '.data | to_entries[] | select(.key | contains("http"))'

# Count total metrics
gcx datasources prometheus metadata -d <uid> -o json | jq '.data | length'

# List all metric names
gcx datasources prometheus metadata -d <uid> -o json | jq '.data | keys[]'
```
...
```bash
# Get all jobs
JOBS=$(gcx datasources prometheus labels -d <uid> --label job -o json)

# Query each job
for job in $(echo $JOBS | jq -r '.data[]'); do
  echo "Job: $job"
  gcx datasources prometheus query <uid> "up{job=\"$job\"}"
done
```
...
gcx datasources prometheus targets -d <uid> --state dropped

# List all targets (active + dropped)
gcx datasources prometheus targets -d <uid> --state any

# Query for down targets
gcx datasources prometheus query <uid> 'up == 0'
```
...
3. Test query to confirm: `gcx datasources prometheus query <uid> 'up{job="system-x"}'`

**For Loki:**
1. List all Loki datasources
2. For each Loki datasource:
   - Get label values for `job`, `namespace`, or `container_name`
   - List series matching the system: `gcx datasources loki series -d <uid> -M '{job="system-x"}'`
...
4. Query series with LogQL selectors:
   ...
   gcx datasources loki series -d <uid> -M '{app="myapp", namespace="production"}'
   ```
...
gcx datasources prometheus query <uid> 'rate(http_requests_total[5m])' --from now-1h --to now --step 1m -o graph
```
