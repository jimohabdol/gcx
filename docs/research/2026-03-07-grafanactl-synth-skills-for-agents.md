# Synthesis Report: Claude Code Skills for grafanactl Synthetic Monitoring
...
| `synth-manage-checks` | **Linear Steps** with decision table (choose type -> configure -> push) | `grafanactl` reference skill |
...
Step 2: Check status (overview or single)
  grafanactl synth checks status              # all checks
  grafanactl synth checks status <ID>         # single check
...
Step 1: Identify the failing check
  ...
  If user named a specific check: grafanactl synth checks status <ID>
...
Step 3: Triage -- scope the failure
  grafanactl synth checks timeline <ID> --window 1h
  -> Show graph to user
  grafanactl synth checks timeline <ID> --window 1h -o json
  -> Analyze for agent:
...
Step 2: Create a new check
  ...
  2d. Push the check
      grafanactl synth checks push <file> --dry-run   # preview first
      grafanactl synth checks push <file>              # apply

Step 3: Update existing checks
  3a. Pull current configs
      grafanactl synth checks pull -d ./checks
  3b. Modify YAML files as needed
  3c. Push changes
      grafanactl synth checks push ./checks/*.yaml --dry-run
      grafanactl synth checks push ./checks/*.yaml

Step 4: GitOps sync (pull/push)
  Pull: grafanactl synth checks pull -d <dir>
  Push: grafanactl synth checks push <files...>
...
The existing CLAUDE.md already documents the `synth` provider's position in the architecture. Skills should not duplicate what CLAUDE.md already covers (per the anti-pattern: "never explain what grafanactl is").
...
1. Use grafanactl commands -- do not call SM APIs directly
...
- Config not set: "Run `grafanactl config set-context` with SM provider settings"
...
When running in agent mode (detected via environment), grafanactl defaults to JSON output. Skills should note this but not change their workflow -- the `-o json` / `-o graph` flags are explicit and override agent mode defaults.
...
| Skill #2: synth-investigate-check | 85% | Domain knowledge is solid (8 failure modes well-documented in Grafana Synthetic Monitoring documentation[7][8]). Uncertainty exists around which PromQL patterns the agent can actually run via `grafanactl query` -- this depends on what Prometheus datasource is available and what metrics exist. The `grafana-investigate-alert` skill provides a strong structural analog[5]. |
...
- **PromQL query availability**: The investigation skill assumes the agent can run `grafanactl query` against a Prometheus datasource that has SM metrics. If the datasource is not configured or metrics are not available, some investigation steps will fail gracefully but provide less value.
...
[4] grafanactl Project Documentation: "AGENTS.md — Agent Entry Point."
...
[9] grafanactl Project Documentation: "SLO Provider and Skills."
